...
pick_next_task:
	...
	//HW2
	task_t* next_pro;
	if ( next != NULL && next->band_music != NULL)
	{
		if ((next->band_music->bass != -1) && (next->band_music->bass != next->pid))
		{
			next_pro = find_task_by_pid(next->band_music->bass);
			if (next_pro != NULL && next_pro->state == TASK_RUNNING)
			{
				next_pro->time_slice = next->time_slice;
